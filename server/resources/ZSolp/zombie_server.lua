...
ZombieLimit = tonumber(get(ResourceName..".MaxZombies")) or 500
ZombieStreaming = tonumber(get(ResourceName..".StreamMethod")) or 1
...
ZombieSpeed = tonumber(get(ResourceName..".Speed")) or 1


--- New Settings by Solpadoin ---

IsDefaultAnims = tonumber(get(ResourceName..".IsDefaultAnim")) or 1
IsBothAnim = tonumber(get(ResourceName..".IsBothAnim")) or 1
AnimFast = get(ResourceName..".AnimFast")
AnimHurt = get(ResourceName..".AnimHurt")
IsZombiesWeak = tonumber(get(ResourceName..".IsZombiesWeak")) or 1
MaxLimitPerPlayer = tonumber(get(ResourceName..".LimitPerPlayer")) or 10
...
function outbreak(startedResource)
	newZombieLimit = math.min(tonumber(get(ResourceName .. ".Zlimit")) or ZombieLimit, ZombieLimit)
...
function RanSpawn_Z ( gx, gy, gz, rot)
	...
	if client then
		if source ~= client or getElementType(client) ~= "player" then
			return
		end
		local px, py, pz = getElementPosition(client)
		if getDistanceBetweenPoints3D(px, py, pz, gx, gy, gz) > 95 then
			return
		end
	end
		...
		if table.getn ( everyZombie ) < newZombieLimit then
			...
			if zomb ~= false then
				if isElement(source) then
					setElementInterior(zomb, getElementInterior(source))
					setElementDimension(zomb, getElementDimension(source))
				end
		...
		if (table.getn( everyZombie )+pacecount < newZombieLimit ) and (ZombieStreaming == 1) then
			...
			if (table.getn( liveplayers ) > 0 ) then
				...
				if isElement(lowestguy) then
					...
					for _, otherPlayer in ipairs(liveplayers) do
						if otherPlayer ~= lowestguy
							and getElementInterior(otherPlayer) == getElementInterior(lowestguy)
							and getElementDimension(otherPlayer) == getElementDimension(lowestguy) then
							local ox, oy, oz = getElementPosition(otherPlayer)
							if getDistanceBetweenPoints3D(px, py, pz, ox, oy, oz) <= 80 then
								clustered = true
								break
							end
						end
					...
					local minimumRadius = clustered and 30 or 15
					local maximumRadius = clustered and 80 or 40
					local angle = math.rad(math.random(0, 359))
					local radius = math.random(minimumRadius, maximumRadius)
					local xcoord = math.cos(angle) * radius
					local ycoord = math.sin(angle) * radius
					triggerClientEvent(lowestguy, "Spawn_Placement", resourceRoot, xcoord, ycoord)
				else
					pacecount = pacecount+1
				end
			else
